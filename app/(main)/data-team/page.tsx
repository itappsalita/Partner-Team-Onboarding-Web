...
function DataTeamContent() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (projectIdDropdownRef.current && !projectIdDropdownRef.current.contains(e.target as Node)) {
        setProjectIdDropdownOpen(false);
        setProjectIdSearch("");
      }
      if (provinsiDropdownRef.current && !provinsiDropdownRef.current.contains(e.target as Node)) {
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchAsg, searchPartner, filterProvinsis, filterProjectIds, activeTab]);
  ...
  const ALLOWED_FILE_TYPES = ["application/pdf", "application/vnd.ms-excel", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"];
  ...
  const filteredDataTeams = dataTeams.filter(dt => {
    ...
    // 2. Search Nomor ASG
    if (searchAsg && !dt.displayId?.toLowerCase().includes(searchAsg.toLowerCase())) return false;

    // 3. Search Partner
    if (searchPartner && !dt.partner?.name?.toLowerCase().includes(searchPartner.toLowerCase())) return false;

    // 4. Filter Provinsi (multi-select)
    if (filterProvinsis.length > 0 && !filterProvinsis.includes(dt.request?.provinsi ?? "")) return false;

    // 5. Filter Project ID (multi-select)
    if (filterProjectIds.length > 0 && !filterProjectIds.includes(dt.request?.siteId ?? "")) return false;
  ...
  return (
    <div className="animate-in fade-in transition-all duration-500">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center gap-4 mb-6">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 flex-1 w-full">
          <div className="relative group">
            <input
              ...
              value={searchAsg}
              onChange={(e) => setSearchAsg(e.target.value)}
            />
          ...
          <div className="relative" ref={provinsiDropdownRef}>
            <button
              type="button"
              onClick={() => setProvinsiDropdownOpen(o => !o)}
              className={`w-full pl-10 pr-4 py-3 bg-alita-white border rounded-xl text-xs font-bold text-left focus:outline-none transition-all shadow-sm flex items-center justify-between ${
                provinsiDropdownOpen ? "border-alita-orange ring-4 ring-alita-orange-glow" : "border-alita-gray-100"
              }`}
            >
              <span className={filterProvinsis.length > 0 ? "text-alita-black" : "text-alita-gray-400"}>
                {filterProvinsis.length === 0
                  ? "Semua Provinsi"
                  : filterProvinsis.length === 1
                  ? filterProvinsis[0]
                  : `${filterProvinsis.length} Provinsi dipilih`}
              </span>
              <svg className={`w-3.5 h-3.5 text-alita-gray-400 transition-transform ${provinsiDropdownOpen ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-alita-gray-300 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>

            {provinsiDropdownOpen && (
              <div className="absolute z-20 mt-1 w-full bg-alita-white border border-alita-gray-100 rounded-xl shadow-lg overflow-hidden">
                <div className="p-2 border-b border-alita-gray-100">
                  <div className="relative">
                    <input
                      type="text"
                      autoFocus
                      placeholder="Ketik nama provinsi..."
                      className="w-full pl-8 pr-3 py-2 bg-alita-gray-50 border border-alita-gray-200 rounded-lg text-xs font-bold focus:outline-none focus:border-alita-orange transition-all"
                      value={provinsiSearch}
                      onChange={(e) => setProvinsiSearch(e.target.value)}
                    />
                    <svg className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-alita-gray-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
                  </div>
                </div>
                <ul className="max-h-60 overflow-y-auto py-1">
                  {PROVINSI_INDONESIA.filter(p => p.toLowerCase().includes(provinsiSearch.toLowerCase())).map(p => {
                    const checked = filterProvinsis.includes(p);
                    return (
                      <li key={p}>
                        <button
                          type="button"
                          onClick={() => setFilterProvinsis(prev =>
                        ...
                        >
                          <span className={`w-4 h-4 shrink-0 rounded border-2 flex items-center justify-center transition-colors ${checked ? "bg-alita-orange border-alita-orange" : "border-alita-gray-300"}`}>
                            {checked && <svg className="w-2.5 h-2.5 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3.5"><polyline points="20 6 9 17 4 12"/></svg>}
                          </span>
                          <span className="text-xs font-bold text-alita-black">{p}</span>
                        </button>
                      </li>
                    );
                  })}
                  {PROVINSI_INDONESIA.filter(p => p.toLowerCase().includes(provinsiSearch.toLowerCase())).length === 0 && (
                    <li className="px-4 py-3 text-xs text-alita-gray-400 italic">Tidak ditemukan</li>
                  )}
                </ul>
                {filterProvinsis.length > 0 && (
                  <div className="border-t border-alita-gray-100 px-4 py-2">
                    <button
                      type="button"
                      onClick={() => setFilterProvinsis([])}
          ...
          <div className="relative" ref={projectIdDropdownRef}>
            <button
              type="button"
              onClick={() => setProjectIdDropdownOpen(o => !o)}
              className={`w-full pl-10 pr-4 py-3 bg-alita-white border rounded-xl text-xs font-bold text-left focus:outline-none transition-all shadow-sm flex items-center justify-between ${
                projectIdDropdownOpen ? "border-alita-orange ring-4 ring-alita-orange-glow" : "border-alita-gray-100"
              }`}
            >
              <span className={filterProjectIds.length > 0 ? "text-alita-black" : "text-alita-gray-400"}>
                {filterProjectIds.length === 0
                  ? "Semua Project ID"
                  : filterProjectIds.length === 1
                  ? filterProjectIds[0]
                  : `${filterProjectIds.length} Project ID dipilih`}
              </span>
              <svg className={`w-3.5 h-3.5 text-alita-gray-400 transition-transform ${projectIdDropdownOpen ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="6 9 12 15 18 9"/></svg>
            </button>
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-alita-gray-300 pointer-events-none" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M9 21V9"/></svg>

            {projectIdDropdownOpen && (
              <div className="absolute z-20 mt-1 w-full bg-alita-white border border-alita-gray-100 rounded-xl shadow-lg overflow-hidden">
                {projectIdOptions.length === 0 ? (
                  <div className="px-4 py-3 text-xs text-alita-gray-400 italic">Belum ada Project ID</div>
                ) : (
                  <>
                    <div className="p-2 border-b border-alita-gray-100">
                      <div className="relative">
                    ...
                    <ul className="max-h-60 overflow-y-auto py-1">
                      {projectIdOptions.filter(p =>
                        p.projectId.toLowerCase().includes(projectIdSearch.toLowerCase()) ||
                        p.projectName.toLowerCase().includes(projectIdSearch.toLowerCase())
                      ).map(p => {
                        const checked = filterProjectIds.includes(p.projectId);
                        return (
                          <li key={p.id}>
                            <button
                              type="button"
                              onClick={() => setFilterProjectIds(prev =>
                                checked ? prev.filter(id => id !== p.projectId) : [...prev, p.projectId]
                              )}
                            ...
                            >
                              ...
                              <span className="flex flex-col min-w-0">
                                <span className="text-xs font-black text-alita-black">{p.projectId}</span>
                                <span className="text-[10px] text-alita-gray-400 truncate">{p.projectName}</span>
                              </span>
                            </button>
                          </li>
                        );
        ...
        {(searchAsg || searchPartner || filterProvinsis.length > 0 || filterProjectIds.length > 0) && (
          <button
            onClick={() => {
              setSearchAsg("");
              setSearchPartner("");
              ...
              {loading ? (
                ...
                currentItems.map((dt) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-5">
                      {dt.request?.siteId
                        ? <span className="px-2.5 py-1 bg-alita-gray-50 border border-alita-gray-200 rounded-full text-[10px] font-black text-alita-gray-600 tracking-wider">{dt.request.siteId}</span>
      ...
      <Modal isOpen={isEditDocsModalOpen} onClose={() => setIsEditDocsModalOpen(false)} title="Update Dokumen Penunjukan (TOR & BAK)">
        <form onSubmit={submitEditDocs} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              <label className="text-xs font-black uppercase text-alita-gray-500">Update TOR <span className="normal-case font-medium text-alita-gray-400">(PDF / Excel, maks. 10MB)</span></label>
              <input
                type="file"
                accept=".pdf,.xls,.xlsx"
                ...
                onChange={(e) => {
                  ...
                  setEditFiles({ ...editFiles, tor: file });
                }}
              />
            </div>
            <div className="flex flex-col gap-2">
              <label className="text-xs font-black uppercase text-alita-gray-500">Update BAK <span className="normal-case font-medium text-alita-gray-400">(PDF / Excel, maks. 10MB)</span></label>
              <input
                type="file"
                accept=".pdf,.xls,.xlsx"
                className="w-full text-xs file:mr-3 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-[11px] file:font-semibold file:bg-alita-gray-100 file:text-alita-gray-700 hover:file:bg-alita-gray-200 italic text-alita-gray-400"
                onChange={(e) => {
                  ...
                  setEditFiles({ ...editFiles, bak: file });
                }}
              />
            </div>
          </div>
